...
function SetupWizard() {
  ...
  const saveBranding = async (markComplete = false) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Falha ao salvar marca");
      throw error;
    } finally {
      setSaving(false);
    }
  ...
  const activateLicense = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Falha ao ativar licença");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="mx-auto flex min-h-screen max-w-xl flex-col justify-center gap-8 px-6 py-12">
        ...
        {step === "checks" && (
          <div className="space-y-4 rounded-xl border border-border p-6">
            ...
            {isError && (
              <p className="text-destructive">API indisponível. Suba o compose e tente de novo.</p>
            )}
            {status && (
              <ul className="space-y-1 text-sm">
                <li>API: ok</li>
                <li>Usuários: {status.hasUsers ? "já existem" : "nenhum ainda"}</li>
                <li>Setup: {status.setupCompleted ? "já concluído" : "pendente"}</li>
              </ul>
            )}
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => refetch()}>Reverificar</Button>
              <Button type="button" disabled={isLoading || isError} onClick={() => setStep("brand")}>
                Continuar
              </Button>
            </div>
          </div>
        ...
        {step === "brand" && (
          <div className="space-y-4 rounded-xl border border-border p-6">
            ...
            <div className="rounded-lg border border-border p-4" style={{ borderColor: primaryColor }}>
            ...
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => setStep("checks")}>Voltar</Button>
              <Button
                type="button"
        ...
        {step === "license" && (
          <div className="space-y-4 rounded-xl border border-border p-6">
            ...
            <p className="text-sm text-muted-foreground">
              Cole a chave ElseTasks Local (ex.: ET-LOCAL-…). Você pode pular e ativar depois.
            </p>
            <Input
            ...
            />
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => setStep("brand")}>Voltar</Button>
              <Button
                type="button"
                variant="outline"
                disabled={saving}
          ...
          <div className="space-y-4 rounded-xl border border-border p-6">
            ...
            <div className="flex gap-2">
              <Button
                type="button"
                onClick={() =>
                  navigate({ to: status?.hasUsers ? "/auth/sign-in" : "/auth/sign-up" })
                }
              >
